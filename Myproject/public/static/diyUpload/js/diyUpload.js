...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			$.each( getOption( '#'+$fileInputId ),function( key, value ){
                    ...
                    $("#Discuss_Buttom").css("width","300px;");
			});
			...
			webUploader.on('uploadFinished', function(){
				...
				$.ajax({
					type: "POST",
					url: "/admin/discuss/Discuss/TextUpload",
					...
					success: function(msg){
						if(msg==="success") {
							$.showBox("发表成功");
                            window.location.href="/admin/discuss/Discuss/index";
						}
						else
						{
                            $.showBox("发表失败!");
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//创建按钮
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {//<a class="diyStart" href="javascript:void(0)">开始上传</a>
